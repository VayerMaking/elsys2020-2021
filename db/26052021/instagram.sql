...
select owner.name as "owner",
komentiral.name as "komentiral"
from Post
right join Komentar
on Komentar.post_id = Post.id
left join Profile as komentiral
on Komentar.profile_id = komentiral.id
left join Profile as owner
on owner.id = owner.id;
...
where Komentar.post_id is null;
...
where Komentar.post_id is null or Llike.post_id is null;
